    ...
    <div className="card-tile p-4 flex flex-wrap items-center gap-3">
      ...
      {active && (
        <button
          type="button"
          onClick={onReset}
          className="ml-auto inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.2em] text-rose-500 hover:bg-rose-50 px-3 py-1.5 rounded-xl"
        >
          <X className="w-3 h-3" />
          Reset Filter
        </button>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {divisions.map((d) => {
        ...
        return (
          <div
            key={d.id}
            className="group card-tile card-hover p-5 relative"
          >
            <button
              type="button"
              onClick={() => onOpen(d)}
...
function KpiCardDetail({ card, onBack, onEditCard, onDeleteCard }: {
  ...
  return (
    <div className="space-y-6">
      <div className="card-flat p-5 flex flex-wrap items-center justify-between gap-4">
    ...
    <div className="card-flat overflow-hidden shadow-sm">
      <div className="flex items-center justify-between px-5 py-3 bg-emerald-50 border-b border-emerald-100">
        ...
        <button
          type="button"
          onClick={onAddMetric}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white text-emerald-700 hover:bg-emerald-100 border border-emerald-200 text-[10px] font-black uppercase tracking-[0.2em]"
        >
