...
const COLORS = ['#FFE53D', '#FF6B9D', '#5BC0EB', '#A6E22E', '#B388EB', '#FF8C42'];

export const TopProducts = ({ products }: Props) => {
  const sorted = [...products].sort((a, b) => b.revenue - a.revenue).slice(0, 6);
  ...
  return (
    <Card variant="paper" className="flex flex-col gap-3 h-full">
      <div className="flex items-center gap-2">
        <Badge variant="cyan">Top Products</Badge>
      </div>
          ...
          {sorted.map((p, i) => {
            ...
            return (
              <div key={p.name} className="flex items-start gap-2.5">
                <span
                  className="w-6 h-6 flex items-center justify-center text-white text-xs font-display flex-shrink-0 border-2 border-nb-black"
                  style={{ background: COLORS[i % COLORS.length] }}
                >
                  <span className="text-nb-black">{i + 1}</span>
                </span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between gap-2 mb-1">
                    <p className="text-xs md:text-sm font-bold leading-tight line-clamp-2 break-words">
                      {p.name}
                    </p>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-white border border-nb-black overflow-hidden">
                      <div
                        className="h-full bg-nb-black"
                        style={{ width: `${clamp(pct, 0, 100)}%` }}
                      />
                    </div>
                    <span className="text-[10px] font-bold tabular-nums text-nb-black/60 w-10 text-right">
